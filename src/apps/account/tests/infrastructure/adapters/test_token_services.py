...
from src.apps.account.infrastructure.token_services import SimpleJwtTokenService


# 계약: 토큰 발급 시, SimpleJwtTokenService는 올바른 User 모델을 찾아 토큰을 생성해야 한다.
def test_issue_for_user(mocker):
    mock_user = MagicMock()
    mock_get_user = mocker.patch('src.apps.account.infrastructure.token_services.Users.objects.get', return_value=mock_user)
    ...
    mock_for_user = mocker.patch('src.apps.account.infrastructure.token_services.RefreshToken.for_user', return_value=mock_refresh_token)
...
def test_blacklist_token_successfully(mocker):
    ...
    mock_refresh_token_class = mocker.patch('src.apps.account.infrastructure.token_services.RefreshToken')
