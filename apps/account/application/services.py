...
class UserAuthAppService:
    def __init__(self,
                 ...
                 social_verifier_map: Dict[str, SocialTokenVerifier],
    ...
    def login_or_register(self, request_dto: SocialLoginRequestDto) -> AuthResponseDto:
        ...
        verified_info = verifier.verify(request_dto.email)

        social_link_vo = SocialLink(provider_name=provider, social_id=verified_info.social_id)
        email_vo = Email(verified_info.email)

        existing_account = self.user_account_repository.find_by_social_link(social_link_vo)
        is_new_user = False
        ...
        if existing_account:
            ...
            if existing_user_by_email:
                user_account = existing_user_by_email
            ...
            tokens = self.token_service.issue_for_user(saved_user_account)
            user_dto = self._map_domain_to_dto(user_account)
